...
export function buildSystemPrompt(vector: Objective[]): string {
  ...
  lines.push(
    ...
    "",
    "Each score carries a confidence (high, medium, or low). A low-confidence number rests on estimated",
    "data rather than measured, so weigh it accordingly and say so plainly if you rely on it.",
    "",
